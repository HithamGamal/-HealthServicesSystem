            ...
            try
            {
                ...
                using (dbContext db = new dbContext())
                {
                    if (UserName.Text != "")
                    {
                            ...
                            int x = int.Parse(UserId.Text);
                            int userrID = LoginForm.Default.UserId;
                        ...
                        GrdPermIssions.DataSource = u;
                            if (u.Count > 0)
                            {
                                ...
                                {
                                    var v1 = db.UserPermissions.Where(p => p.UserId == userrID).Select(x1 => x1.FormId).ToArray();
                                    var otherObjects =
                                        db.SysForms.Where(x1 =>
                                            // x1.SystemsId ==  SysId && 
                                            v1.Contains(x1.Id))
                            ...
                            else
                            {
                                ...
                                if (grp.Count > 0)
                                {
                                    ...
                                    if (v.Count > 0)
                                    {
                                        ...
                                        var v1 =
                                            db.UserPermissions.Where(p => p.UserId == userrID).Select(x1 => x1.FormId).ToArray();
                                        var otherObjects =
                                            db.SysForms.Where(x1 => 
                                            //x1.SystemsId == SysId && 
                                            v1.Contains(x1.Id))
