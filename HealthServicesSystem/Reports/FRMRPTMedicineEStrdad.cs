            ...
            if (UserId == 44)
            {
                if (SirkNo.Text.Length > 0)
                {
                    ...
                    using (dbContext db = new dbContext())
                    {
                        ...
                        var GetDet = db.Database.SqlQuery<ReportForAll>(StrRPT2).Where(p => p.Row23==SrkNo).OrderBy(p => p.Row13).ToList();
                        //MessageBox.Show(GetDet.Count.ToString());
                        if (GetDet.Count > 0)
                        {
                            Cursor = Cursors.WaitCursor;
